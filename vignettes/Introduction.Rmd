---
title: "Introduction to StreamCatTools"
author: "Marc Weber"
output: 
  html_document:
    theme: flatly
...
```{r API}
df <- readr::read_csv("https://v26267mcpk506/StreamCat/v1/stable/metrics?name=fert&areaOfInterest=watershed&comid=179", show_col_types = FALSE)
knitr::kable(df)
```
...
temp_dir <- '.'
nhdplus <- nhdplusTools::subset_nhdplus(comids = flowline_nldi$UT_flowlines$nhdplus_comid, output_file = file.path(temp_dir, "nhdplus.gpkg"),nhdplus_data = "download",overwrite = TRUE,flowline_only = FALSE)
