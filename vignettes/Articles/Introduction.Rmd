...
## Get COMIDs
In this example we use the `sc_get_comid` function to find COMIDs for USGS stream gages we load into R. We convert the csv to an sf POINT layer and call `sc_get_comid()` which batches queries to the EPA ArcGIS catchments service.
```{r comids, eval=FALSE}
...
# batch lookup (uses the package default ArcGIS catchments layer)
gages_coms <- sc_get_comid(gages_sf)

# attach COMIDs back to the original data frame
gages$COMID <- as.character(gages_coms)
# request a small set of StreamCat metrics for the found COMIDs
df <- sc_get_data(metric='huden2010', aoi='ws', comid = paste(gages_coms, collapse = ","))
