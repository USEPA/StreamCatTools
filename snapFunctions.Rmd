...
# Ideally we would combine them now to output to a user, but can't
...
That function is where I started before going to a list output. I can feed the snap_Points_to_Feature() points that will and wont connect to a segment within a given buffer distance, but the output is difficult to use. As you can see from line 153, I tried to get rid of the template row before outputting result from function, but error messages occurred.

```{r troubleshooting}
z <- snap_Points_to_Feature(probSites_sf[1:3,],'StationID',WQS, seq(10,50,by=10))

# trying to get rid of template row
filter(z, `Point Unique Identifier` != 'deleteMe') # fail
z[-1,] # fail

```


Working function outputs a list with one sf object for things that connected to segment within buffer and one smaller tbl that identifies sites that need attention. I didn't love this initally, but I think it offers a somewhat elegant solution to the problem because a single sf output with sites that worked and ones that didn't would still need to be filtered to get rid of empty geometry rows before manipulating further.
