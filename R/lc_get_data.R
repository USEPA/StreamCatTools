...
# Fast check: Arrow S3 enabled?
stopifnot(arrow::arrow_with_s3())

# Connect to the bucket (adjust region; add requester_pays = TRUE if needed)
fs <- s3_bucket("dmap-data-commons-ow", anonymous = TRUE, region = "us-east-1")

# Open the single GeoParquet file
ds <- open_dataset("streamcat/LakeCatWatersheds", filesystem = fs, format = "parquet")

# Filter + project (only columns you need)
comid_list <- c(23783629, 23794487, 23812618)
df <- ds |>
  select(COMID, geometry, GNIS_ID, GNIS_NAME, REACHCODE) |>
  filter(COMID %in% comid_list) |>
  collect()  # only filtered rows and selected columns are fetched
...
dataset <- open_dataset("D:/LakeCatWatersheds_4326.parquet")
comid_list <- c(23783629,23794487,23812618)
df <- dataset |>
  filter(COMID %in% comid_list) |>
  collect()
...
class(df$geometry)
# [1] "geoarrow_wkb" "vctrs_vctr"   ... (expected)
wk::wk_crs(df$geometry)
# Should show EPSG:4326 or similar if present in metadata

# 1) Get CRS from the geoarrow column if available
crs_meta <- tryCatch(wk::wk_crs(df$geometry), error = function(e) NULL)

# 2) Convert the geoarrow vector to plain WKB (list of raw vectors)
g_wkb <- wk::as_wkb(df$geometry)

# (Optional sanity checks)
stopifnot(is.list(g_wkb), length(g_wkb) == nrow(df))
stopifnot(length(g_wkb) == 0 || inherits(g_wkb[[1]], "raw"))

# 3) Make an sfc from WKB. EWKB=TRUE is safe (it will use SRID if present)
geom_sfc <- sf::st_as_sfc(g_wkb, EWKB = TRUE)

# 4) Apply CRS (from metadata if present, otherwise set explicitly)
if (!is.null(crs_meta)) {
  sf::st_crs(geom_sfc) <- crs_meta
} else {
  sf::st_crs(geom_sfc) <- 4326  # your filename suggests EPSG:4326
}

# 5) Replace the geometry column and create an sf
df$geometry <- geom_sfc
filtered_sf <- sf::st_as_sf(df, sf_column_name = "geometry")

filtered_sf
sf::st_crs(filtered_sf)