...
## Example One
Using `StreamCatTools`, get a list of available StreamCat values for certain parameters using the `get_streamcat_params` function (right now just metric names and areas of interest for this function) via the API
```{r Example One, warning=FALSE, error=FALSE, message=FALSE, include=FALSE}
...
## Example Three
Access a couple watershed-only variables for the area of interest of a county (Benton County in this case) using the `get_streamcat_data` function. 
...
## Example Four
Use the `streamcat_get_comid` function to find comids for a point shapefile we load into R. Note that we could also use a .csv file with coordinate columns and a known coordinate reference system.  We'll use an example dataset of large dams in Oregon from the [Oregon Spatial Data Library]
```{r Example Four, warning=FALSE, error=FALSE, message=FALSE}
...
st_layers('oregon_dams_2015/Oregon_dams_20150107.gdb')
dams <- read_sf(dsn="oregon_dams_2015/Oregon_dams_20150107.gdb",layer='or_dams') 
# glimpse(dams)
