...
```{r calapooia, fig.cap="Map of NLCD percent imperviousness for each catchment for the Calapooia River watershed in Oregon.", warning=FALSE, message=FALSE, echo=FALSE, label="watershed map",  out.width="95%", fig.align="center"}
calapooia <- ggplot() +
    geom_sf(data = basin,
            fill = NA,
            color = "black",
...
```{r landcover, fig.cap="NLCD land cover proportions with an example lake watershed.", warning=FALSE, message=FALSE, echo=FALSE, label="NLCD figure",  out.width="95%", fig.align="center"}
df <- df |>
  dplyr::mutate(
...
```{r NNI, fig.cap="Annual time series of nitrogen and phosphorus buget data for the Mississippi=Atchafalaya river basin.", warning=FALSE, message=FALSE, label="NNI figure",  out.width="95%", fig.align="center"}
library(StreamCatTools)
library(ggplot2)
library(ggpattern)
com <- '22812041'
